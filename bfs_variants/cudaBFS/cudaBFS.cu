// nvcc -std=c++17 -O3 cudaBFS.cu -o cudaBFS
#include <iostream>
#include <vector>
#include <queue>
#include <fstream>
...
void BFS(const int s, const int n, const std::vector<std::vector<int>> &adjlist, std::vector<int>& dist)
{
    ...
    auto csr_duration = std::chrono::duration<double, std::milli>(csr_end - csr_start).count();
    ...
    int *d_flag;   // Device flag
    int *h_flag;   // Host flag
    ...
    // Initialize the flag on host and copy to device
    *h_flag = 0;
    ...
    *h_flag = 1;
    while(*h_flag)
    {
      *h_flag = 0;
      ...
      (n, c_iteration_no, c_edgelist, c_csr_edge_range, c_dist, c_parent, c_visited, d_flag);
      cudaDeviceSynchronize();

      // Copy flag back to host to check termination condition
    ...
    auto new_duration = std::chrono::duration<double, std::milli>(new_end - new_start);
...
int main(int argc, char* argv[]) { 
  ...
  std::cout << "Time for reading input : " << duration.count() << " microseconds.\n";
  
  std::vector<int> dist(n, INT_MAX);
  std::cout <<"Started cuda BFS.\n";
